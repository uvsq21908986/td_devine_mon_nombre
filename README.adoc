...
git add .
git commit -m "Ajout du programme devine_mon_nombre en Rust"
🡄
....
...
+
....
🡆 La notation &mut permet d'indiquer qu'une référence est mutable et peut donc être modifiée.🡄
....
.. Quel est le type de retour de `read_line` ? Quel est son utilité ?
+
....
🡆 Le type de retour de read_line est Result<usize, std::io::Error>. Il est utilisé pour signaler si la lecture a réussi ou non. Si tout s'est bien passé, read_line renverra Ok(n) où n est le nombre d'octets lus. Sinon, elle renverra Err(e) où e est l'erreur rencontrée.🡄
....
...
+
....
🡆 Cargo.toml, Cargo.toml, src/main.rs 🡄
....
